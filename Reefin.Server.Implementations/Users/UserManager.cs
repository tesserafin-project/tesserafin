...
namespace Reefin.Server.Implementations.Users
{
    ...
    public partial class UserManager : IUserManager, IDisposable
    {
        ...
        private static IQueryable<User> UserQuery(ReefinDbContext dbContext)
        {
            return dbContext.Users
                            .AsSingleQuery()
...
#pragma warning restore CA1862 // Use the 'StringComparison' method overloads to perform case-insensitive string comparisons
        ...
        public async Task UpdateUserAsync(User user)
        {
            using (await _userLock.LockAsync(user.Id).ConfigureAwait(false))
            {
                ...
                await using (dbContext.ConfigureAwait(false))
                {
                    ...
                    {
                        dbUser.ProfileImage = new Reefin.Database.Implementations.Entities.ImageInfo(user.ProfileImage.Path)
                        {
        ...
        internal async Task<User> CreateUserInternalAsync(string name, ReefinDbContext dbContext)
        {
...
#pragma warning restore CA1862 // Use the 'StringComparison' method overloads to perform case-insensitive string comparisons
        ...
        private async Task UpdateUserInternalAsync(ReefinDbContext dbContext, User user)
        {
