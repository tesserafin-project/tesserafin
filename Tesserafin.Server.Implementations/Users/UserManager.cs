...
namespace Tesserafin.Server.Implementations.Users
{
    /// <summary>
    ...
    public partial class UserManager : IUserManager, IDisposable
    {
        ...
        private static IQueryable<User> UserQuery(TesserafinDbContext dbContext)
        {
            return dbContext.Users
                            .AsSingleQuery()
...
#pragma warning restore CA1862 // Use the 'StringComparison' method overloads to perform case-insensitive string comparisons
        ...
        public async Task UpdateUserAsync(User user)
        {
            using (await _userLock.LockAsync(user.Id).ConfigureAwait(false))
            {
                ...
                await using (dbContext.ConfigureAwait(false))
                {
                    ...
                    {
                        dbUser.ProfileImage = new Tesserafin.Database.Implementations.Entities.ImageInfo(user.ProfileImage.Path)
                        {
        ...
        internal async Task<User> CreateUserInternalAsync(string name, TesserafinDbContext dbContext)
        {
...
#pragma warning restore CA1862 // Use the 'StringComparison' method overloads to perform case-insensitive string comparisons
        ...
        private async Task UpdateUserInternalAsync(TesserafinDbContext dbContext, User user)
        {
