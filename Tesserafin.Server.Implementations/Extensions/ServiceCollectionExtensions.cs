...
using Tesserafin.Database.Implementations.DbConfiguration;
using Tesserafin.Database.Implementations.Locking;
using Tesserafin.Database.Providers.Sqlite;
using TesserafinDbProviderFactory = System.Func<System.IServiceProvider, Tesserafin.Database.Implementations.ITesserafinDatabaseProvider>;

namespace Tesserafin.Server.Implementations.Extensions;
...
public static class ServiceCollectionExtensions
{
    ...
    private static IDictionary<string, TesserafinDbProviderFactory> GetSupportedDbProviders()
    {
        var items = new Dictionary<string, TesserafinDbProviderFactory>(StringComparer.InvariantCultureIgnoreCase);
        foreach (var providerType in DatabaseProviderTypes())
        {
            var keyAttribute = providerType.GetCustomAttribute<TesserafinDatabaseProviderKeyAttribute>();
            ...
            items[keyAttribute.DatabaseProviderKey] = (services) => (ITesserafinDatabaseProvider)ActivatorUtilities.CreateInstance(services, providerType);
        }

        return items;
    }

    private static TesserafinDbProviderFactory? LoadDatabasePlugin(CustomDatabaseOptions customProviderOptions, IApplicationPaths applicationPaths)
    {
        ...
        var dbProviderType = assembly.GetExportedTypes().FirstOrDefault(f => f.IsAssignableTo(typeof(ITesserafinDatabaseProvider)))
            ?? throw new InvalidOperationException($"Could not find any type implementing the '{nameof(ITesserafinDatabaseProvider)}' interface.");

        return (services) => (ITesserafinDatabaseProvider)ActivatorUtilities.CreateInstance(services, dbProviderType);
    }
    ...
    public static IServiceCollection AddTesserafinDbContext(
        ...
        IConfiguration configuration)
    {
        ...
        TesserafinDbProviderFactory? providerFactory = null;
        ...
        serviceCollection.AddSingleton<ITesserafinDatabaseProvider>(providerFactory!);
        ...
        serviceCollection.AddPooledDbContextFactory<TesserafinDbContext>((serviceProvider, opt) =>
        {
            var provider = serviceProvider.GetRequiredService<ITesserafinDatabaseProvider>();
