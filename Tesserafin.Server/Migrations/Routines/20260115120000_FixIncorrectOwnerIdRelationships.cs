...
/// </summary>
[TesserafinMigration("2026-01-15T12:00:00", nameof(FixIncorrectOwnerIdRelationships))]
...
public class FixIncorrectOwnerIdRelationships : IAsyncMigrationRoutine
{
    ...
    private async Task RemoveDuplicateItemsAsync(TesserafinDbContext context, CancellationToken cancellationToken)
    {
        ...
        foreach (var path in duplicatePaths)
        {
            ...
            var itemToKeep = itemsWithPath
                ...
                .ThenByDescending(i => i.Type != "Tesserafin.Controller.Entities.Folder")
    ...
    private async Task ClearIncorrectOwnerIdsAsync(TesserafinDbContext context, CancellationToken cancellationToken)
    {
        ...
        var incorrectChildrenWithParent = await context.BaseItems
            .Where(b => b.OwnerId.HasValue
                ...
                && (b.Type == "Tesserafin.Controller.Entities.Video" || b.Type == "Tesserafin.Controller.Entities.Movies.Movie"))
            .Where(b => context.BaseItems.Any(parent =>
                parent.Id.Equals(b.OwnerId!.Value)
                && (parent.Type == "Tesserafin.Controller.Entities.Video" || parent.Type == "Tesserafin.Controller.Entities.Movies.Movie")))
            .ToListAsync(cancellationToken)
            .ConfigureAwait(false);
    ...
    private async Task ReassignOrphanedExtrasAsync(TesserafinDbContext context, CancellationToken cancellationToken)
    {
        ...
        var videoTypes = new[]
        {
            "Tesserafin.Controller.Entities.Video",
            "Tesserafin.Controller.Entities.Movies.Movie"
        };
    ...
    private async Task PopulatePrimaryVersionIdAsync(TesserafinDbContext context, CancellationToken cancellationToken)
    {
        ...
        var alternateVersionLinks = await context.LinkedChildren
            .Where(lc => (lc.ChildType == Tesserafin.Database.Implementations.Entities.LinkedChildType.LocalAlternateVersion
                       || lc.ChildType == Tesserafin.Database.Implementations.Entities.LinkedChildType.LinkedAlternateVersion))
            .Join(
                context.BaseItems,
                lc => lc.ChildId,
                item => item.Id,
