...
using BaseItemEntity = Tesserafin.Database.Implementations.Entities.BaseItemEntity;
using Chapter = Tesserafin.Database.Implementations.Entities.Chapter;
...
/// </summary>
[TesserafinMigration("2025-04-20T20:00:00", nameof(MigrateLibraryDb))]
[TesserafinMigrationBackup(TesserafinDb = true, LegacyLibraryDb = true)]
internal class MigrateLibraryDb : IDatabaseMigrationRoutine
{
    ...
    private readonly IDbContextFactory<TesserafinDbContext> _provider;
    ...
    /// <param name="reefinDatabaseProvider">The database provider for special access.</param>
    public MigrateLibraryDb(
        IStartupLogger<MigrateLibraryDb> startupLogger,
        IDbContextFactory<TesserafinDbContext> provider,
        IServerApplicationPaths paths,
        ITesserafinDatabaseProvider reefinDatabaseProvider)
    {
    ...
    public void Perform()
    {
        ...
        {
            operation.TesserafinDbContext.AttachmentStreamInfos.ExecuteDelete();
            operation.TesserafinDbContext.BaseItems.ExecuteDelete();
            operation.TesserafinDbContext.ItemValues.ExecuteDelete();
            operation.TesserafinDbContext.UserData.ExecuteDelete();
            operation.TesserafinDbContext.MediaStreamInfos.ExecuteDelete();
            operation.TesserafinDbContext.Peoples.ExecuteDelete();
            operation.TesserafinDbContext.PeopleBaseItemMap.ExecuteDelete();
            operation.TesserafinDbContext.Chapters.ExecuteDelete();
            operation.TesserafinDbContext.AncestorIds.ExecuteDelete();
        }
        ...
        using (var operation = GetPreparedDbContext("Moving TypedBaseItem"))
        {
            ...
            using (new TrackedMigrationStep("Clean TypedBaseItems hierarchy", _logger))
            {
                ...
                foreach (var item in allItemsLookup)
                {
                    ...
                    if (DoesResolve(cachedItem.BaseItem.ParentId, checkStack))
                    {
                        ...
                        operation.TesserafinDbContext.BaseItems.Add(cachedItem.BaseItem);
            ...
            using (new TrackedMigrationStep($"Saving {operation.TesserafinDbContext.BaseItems.Local.Count} BaseItem entries", _logger))
            {
                operation.TesserafinDbContext.SaveChanges();
            }
        ...
        using (var operation = GetPreparedDbContext("Moving ItemValues"))
        {
            ...
            using (new TrackedMigrationStep("Loading ItemValues", _logger))
            {
                ...
                {
                    operation.TesserafinDbContext.ItemValues.Add(item.Value.ItemValue);
                    operation.TesserafinDbContext.ItemValuesMap.AddRange(item.Value.ItemIds.Distinct().Select(f => new ItemValueMap()
                    {
                        Item = null!,
                        ItemValue = null!,
                        ItemId = f,
            ...
            using (new TrackedMigrationStep($"Saving {operation.TesserafinDbContext.ItemValues.Local.Count} ItemValues entries", _logger))
            {
                operation.TesserafinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving UserData"))
        {
            ...
            {
                var users = operation.TesserafinDbContext.Users.AsNoTracking().ToArray();
                ...
                foreach (var entity in queryResult)
                {
                    ...
                    operation.TesserafinDbContext.UserData.Add(userData);
                }
            }

            legacyBaseItemWithUserKeys.Clear();

            using (new TrackedMigrationStep($"Saving {operation.TesserafinDbContext.UserData.Local.Count} UserData entries", _logger))
            {
                operation.TesserafinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving MediaStreamInfos"))
        {
            ...
            using (new TrackedMigrationStep("Loading MediaStreamInfos", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(mediaStreamQuery))
                {
                    ...
                    operation.TesserafinDbContext.MediaStreamInfos.Add(entity);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.TesserafinDbContext.MediaStreamInfos.Local.Count} MediaStreamInfos entries", _logger))
            {
                operation.TesserafinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving AttachmentStreamInfos"))
        {
            ...
            using (new TrackedMigrationStep("Loading AttachmentStreamInfos", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(mediaAttachmentQuery))
                {
                    ...
                    operation.TesserafinDbContext.AttachmentStreamInfos.Add(entity);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.TesserafinDbContext.AttachmentStreamInfos.Local.Count} AttachmentStreamInfos entries", _logger))
            {
                operation.TesserafinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving People"))
        {
            ...
            using (new TrackedMigrationStep("Loading People", _logger))
            {
                ...
                {
                    operation.TesserafinDbContext.Peoples.Add(item.Value.Person);
                    operation.TesserafinDbContext.PeopleBaseItemMap.AddRange(item.Value.Items.DistinctBy(e => (e.ItemId, e.PeopleId)));
                }

                peopleCache.Clear();
            }

            using (new TrackedMigrationStep($"Saving {operation.TesserafinDbContext.Peoples.Local.Count} People entries and {operation.TesserafinDbContext.PeopleBaseItemMap.Local.Count} maps", _logger))
            {
                operation.TesserafinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving Chapters"))
        {
            ...
            using (new TrackedMigrationStep("Loading Chapters", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(chapterQuery))
                {
                    ...
                    operation.TesserafinDbContext.Chapters.Add(chapter);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.TesserafinDbContext.Chapters.Local.Count} Chapters entries", _logger))
            {
                operation.TesserafinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving AncestorIds"))
        {
            ...
            using (new TrackedMigrationStep("Loading AncestorIds", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(ancestorIdsQuery))
                {
                    ...
                    operation.TesserafinDbContext.AncestorIds.Add(ancestorId);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.TesserafinDbContext.AncestorIds.Local.Count} AncestorId entries", _logger))
            {
                operation.TesserafinDbContext.SaveChanges();
            }
        }
    ...
    {
        public DatabaseMigrationStep(TesserafinDbContext reefinDbContext, string operationName, ILogger logger) : base(operationName, logger)
        {
            TesserafinDbContext = reefinDbContext;
        }

        public TesserafinDbContext TesserafinDbContext { get; }
