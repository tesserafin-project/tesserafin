...
public sealed class TesserafinMigrationAttribute : Attribute
{
    /// <summary>
    /// Initializes a new instance of the <see cref="TesserafinMigrationAttribute"/> class.
    /// </summary>
...
#pragma warning disable CS0618 // Type or member is obsolete
    public TesserafinMigrationAttribute(string order, string name) : this(order, name, null)
#pragma warning restore CS0618 // Type or member is obsolete
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="TesserafinMigrationAttribute"/> class for legacy migrations.
    /// </summary>
    /// <param name="order">The ordering this migration should be applied to. Must be a valid DateTime ISO8601 formatted string.</param>
    /// <param name="name">The name of this Migration.</param>
    ...
    public TesserafinMigrationAttribute(string order, string name, string? key)
    {
        Order = DateTime.Parse(order, CultureInfo.InvariantCulture);
        Name = name;
        Stage = TesserafinMigrationStageTypes.AppInitialisation;
        if (key is not null)
        {
            Key = Guid.Parse(key);
        }
    ...
    /// <summary>
    /// Gets or Sets the stage the annoated migration should be executed at. Defaults to <see cref="TesserafinMigrationStageTypes.CoreInitialisation"/>.
    /// </summary>
    public TesserafinMigrationStageTypes Stage { get; set; } = TesserafinMigrationStageTypes.CoreInitialisation;
