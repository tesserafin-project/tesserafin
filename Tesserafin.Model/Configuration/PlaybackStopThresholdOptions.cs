...
/// <c>Tesserafin.MediaEncoding.Playback</c>) recomputes "is the guard tripped right now?" from the live
...
public class PlaybackStopThresholdOptions
{
    ...
    /// <c>AdapterError</c> (<c>Tesserafin.MediaEncoding.Playback.PlaybackLiveFallbackReason</c>) before
    ...
    /// (<c>Tesserafin.Playback.Dlna.PlaybackExecutionPlanAdapter.ToStreamInfo</c>) - the denominator is
    /// served-by-v2 count plus adapter-error count, not the count of every live request. Not a
    /// <c>cref</c> to either type: this project (<c>Tesserafin.Model</c>) sits below both in the
