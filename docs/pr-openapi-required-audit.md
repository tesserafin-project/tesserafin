...
Statut : **mesure conservée comme documentation ; l'approche C est appliquée**. Ce document est
l'audit qui a servi à choisir entre trois correctifs candidats ; il est gardé pour la comparaison
chiffrée A / B / C qu'il porte, pas comme description de l'état courant.

> **Mise à jour (S51)** — ce qui a changé depuis la rédaction :
> - L'approche C est **livrée** : `Reefin.Server/Filters/PlaybackDecisionRequiredSchemaFilter.cs`,
>   limitée par égalité exacte au namespace `Reefin.Playback.Decision`, produit les 29 entrées sur
>   13 schémas décrites en §3. Le contrat a été **régénéré par `./ci/openapi-generate.sh`**.
> - `OpenApiRequiredContractTests` ne fait plus échouer volontairement le build : il **passe**.
>   `OpenApiRequiredContractInventoryTests` épingle en plus la table exhaustive et les contrôles
>   (types valeur et membres nullables restent optionnels).
> - L'écart 263 / 268 de §6 est **réconcilié** (voir §2.1).
> - Docker était disponible pour ce lot : `./ci/openapi-generate.sh` et `./ci/run.sh` ont tourné.
...
L'issue annonce 263 ; le comptage ci-dessus est **268**. **Les deux sont exacts, sur deux
périmètres différents** — réconcilié depuis :

| prédicat `jq` sur `.components.schemas[]` | schémas |
|---|---|
| `has("properties") and (has("required")\|not)` — périmètre de l'issue #51 | **263** |
| `(.type=="object" or has("properties")) and (has("required")\|not)` — périmètre de cet audit | **268** |

L'écart de 5 est exactement l'ensemble des schémas `type: object` **sans clé `properties`** :
`BasePluginConfiguration` (marqueur vide) et les quatre enveloppes `oneOf`/`discriminator`
`GroupUpdate`, `InboundWebSocketMessage`, `OutboundWebSocketMessage`, `WebSocketMessage`. N'ayant
aucun membre, aucune n'est candidate à un `required`. Le chiffre annexe de l'issue (« parmi eux,
ayant des propriétés non-nullables non-bool : 214 ») se reproduit lui aussi à l'identique, ce qui
...
**Appliqué en S51** : le point 1 seulement (`PlaybackDecisionRequiredSchemaFilter`, restreint à
`Reefin.Playback.Decision`). A et B restent **non appliqués**.

Le point 2 (correctif d'appel dans reefin-web) **reste à faire** et doit précéder la régénération
du SDK web. L'issue #51 le donne pour livré par reefin-web#30, mais dans le checkout reefin-web
consulté en lecture seule pour ce lot, `src/scripts/reefinPlaybackCapabilities.ts:291-293` renvoie
toujours `{ Codec: codec }`, et le commentaire de tête (lignes 40-41) documente encore que les
entrées laissent `VideoRangeTypes` non renseigné. Ce checkout peut être en retard sur le dépôt web
(son spec est épinglé au 2026-07-17) : **à vérifier côté web avant de régénérer le SDK**. Rien de
tout cela ne bloque le présent lot serveur — reefin-web régénère depuis son propre spec épinglé,
que ce changement ne touche pas.
...
- ~~**L'écart 263 (issue) / 268 (mesuré)**~~ — **réconcilié**, voir §2.1 : deux prédicats de
  comptage différents, tous deux exacts, l'écart de 5 étant les schémas `type: object` sans clé
  `properties`.
...
- **La contre-sonde booléens** renvoie 400 (« Error processing request. », échec de résolution de
  ...
  pas été exécuté.
- **Le spec miroir de reefin-web est en retard sur master** — mesuré à nouveau en S51 et confirmé
  au chiffre près : 395 schémas contre 401, **117 schémas structurellement divergents**, 6 absents
  et **0 en trop** (`PlaybackEngineMode`, `PlaybackLiveFallbackReason`, `PlaybackLiveWiringOutcome`,
  `PlaybackOperationalMetricsResponse`, `PlaybackSessionStreamDescriptor`,
  `PlaybackStopThresholdOptions`). Le miroir est donc un **sous-ensemble strict et figé**, pas une
  divergence bidirectionnelle. `PlaybackDecisionVideoCodecCapability` y est identique, ce qui est
  précisément pourquoi cette dérive n'a pas masqué le bug de #51.

  Provenance et mécanisme, établis en S51 : `npm run generate:reefin-sdk`
  (`scripts/generate-reefin-sdk.mjs`, openapi-generator-cli 7.11.0) résout son spec dans cet ordre
  — `$REEFIN_OPENAPI_SPEC`, puis un checkout `../reefin` voisin, puis un serveur de dev, puis la
  copie épinglée `src/lib/reefin-sdk/spec/openapi.json`. **La cause structurelle de la dérive est
  que l'étape « checkout voisin » lit
  `../reefin/tests/Reefin.Server.Integration.Tests/bin/{Debug,Release}/net10.0/openapi.json`,
  c'est-à-dire un artefact de build, et jamais `../reefin/openapi/openapi.json`, le contrat
  canonique.** C'est la même famille d'écart que #48. En regard, `npm run verify:reefin-sdk-fresh`
  ne compare que `generated/` au spec **déjà épinglé** dans le dépôt web : par construction il ne
  peut pas voir un retard sur le serveur, donc la CI web est aveugle à cette dérive. Le miroir
  s'auto-documente d'ailleurs (`spec/version.json` : `generatedAt` 2026-07-17,
  `versionSkewNote`).

  **Hors périmètre de ce lot, et à ne pas noyer dans le diff `required`** : resynchroniser le
  miroir produirait un diff de 117 schémas sans rapport avec #51. À traiter par une issue dédiée
  (« le miroir web se régénère depuis un artefact de build, pas depuis le contrat canonique ») et
  une PR de baseline séparée qui fait d'abord passer le miroir à 401 schémas, avant toute
  régénération du SDK portant les 29 `required` de ce lot.
- ~~**Docker interdit sur ce lot**~~ — **levé en S51** : `./ci/openapi-generate.sh` puis
  `./ci/run.sh` ont été exécutés (bin/ et obj/ purgés au préalable, cf. la note sur les faux verts
  d'analyseurs). Le contrat a été régénéré par l'outil, jamais édité à la main.
