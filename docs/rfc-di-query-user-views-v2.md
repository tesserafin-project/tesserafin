...
- **I2 — chemin d'exécution de la création de vue** : entre l'entrée dans `GetNamedView`/`GetShadowView` et le retour de l'item persisté+enregistré, aucun membre de la SCC n'est invoqué. **Unique exception assumée** : `Lazy<IProviderManager>.Value` pour `QueueRefresh`, évaluée **après** persistance+enregistrement (déclenchement de refresh post-création) — dépendance runtime réelle vers la SCC, identique à celle qui existe déjà dans `LibraryManager` aujourd'hui, donc pas une régression ; PR106b doit vérifier empiriquement quand `.Value` est évalué et l'asserter par test (si `.Value` s'avérait évalué avant la persistance, le design doit être revu, pas l'invariant).
