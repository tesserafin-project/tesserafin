...
- **Clos** : 2026-07-14 (PR111, `docs/pr111-di-closure-audit.md`)
...
**Réévaluation à la clôture (PR111)** : les 4 exceptions runtime listées dans ce RFC et découvertes en implémentation (PR106b : `Lazy<IProviderManager>.Value` post-persistance et le static `UpdateToRepositoryAsync` ; PR108 : `Lazy<IReadOnlyList<ILiveTvService>>.Value` ; PR109 : `Lazy<IItemQueryService>.Value`) sont statuées en détail — chemin retracé, statut, justification, chantier futur nommé le cas échéant — dans `docs/pr111-di-closure-audit.md` (« Réévaluation des 4 exceptions runtime bénies »). Toutes les 4 sont conservées ; l'exception 2 (static `UpdateToRepositoryAsync`) s'avère avoir deux sites d'appel dans `UserViewFactory`, pas un seul comme décrit ci-dessus.
