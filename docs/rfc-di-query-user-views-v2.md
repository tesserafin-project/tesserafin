...
- **Précède** : PR106–PR111 (application de la découpe)
...
- **Point ouvert, non résolu par ce RFC** : l'événement `ItemAdded` (voir mise en garde ci-dessus) n'est plus déclenché si la création de vue passe par `UserViewFactory` au lieu de `LibraryManager.CreateItems`. À trancher en implémentation PR106 (voir §10, risque 6).
...
**Note pour PR111** : supprimer `Lazy<IUserViewManager>` (PR110) retire **1** paramètre, pas 3. Pour atteindre le critère « −3 paramètres » posé par le plan v13, il faut cumuler avec PR107 (`LibraryManager` cesse d'implémenter `IUserRootFolderProvider` — ne retire pas de paramètre de ctor à lui seul, sauf si `LibraryManager` injecte désormais `IUserRootFolderProvider` en le déléguant au lieu de garder sa logique privée, ce qui *ajouterait* un paramètre sauf substitution) et PR106 (si `GetNamedView`/`GetShadowView` sont délégués à `IUserViewFactory` injecté, cela remplace de la logique interne par une dépendance — net neutre ou légèrement positif en paramètres, pas négatif). **Le −3 réaliste vient plutôt de la suppression de `_userViewManagerFactory` (−1) plus deux autres candidats indépendants de ce chantier** (ex. `dotIgnoreIgnoreRule`, `mediaStreamLanguageService` si leur logique est absorbée ailleurs) — **ce RFC ne peut garantir ces deux paramètres additionnels sans travail hors périmètre**. Signalé comme risque §9.

---

## 8. Règle d'or et application port par port

**Règle d'or** : aucun nouveau port introduit par PR106–111 ne doit avoir de dépendance transitive vers `LibraryManager` — y compris via un port dont l'implémentation est `LibraryManager` (l'alias `IUserRootFolderProvider`) ou via un fallback statique (`BaseItem.LibraryManager`). Pour chaque port, ce RFC répond à trois questions : (a) qui l'implémente (classe nommée, jamais un alias/factory castant un autre service), (b) quelles sont ses dépendances directes, (c) est-ce que l'une d'elles, transitivement, atteint `LibraryManager`/`UserViewManager`/`ChannelManager`/`LiveTvManager` ?

| Port | Implémentation | Dépendances directes | Atteint la SCC ? |
| --- | --- | --- | --- |
| `IItemStore` (prérequis PR106) | `ItemStore` (nouveau) | `IItemPersistenceService` (→ `ItemPersistenceService`, vérifié), `IItemCacheStore` (→ `ItemLookupService`, vérifié), `IServerConfigurationManager` | Non — implémentations concrètes vérifiées (pas seulement les interfaces), aucune n'implémente/référence un membre de la SCC |
| `IUserViewFactory` (PR106) | `UserViewFactory` (nouveau) | `IItemLookupService`, `IItemStore`, `IServerConfigurationManager`, `IFileSystem`, `Lazy<IProviderManager>` | Non — **à condition que `IProviderManager` soit `Lazy`** ; `ProviderManager` référence `ILibraryManager` en direct (`ProviderManager.cs:117`), donc non-Lazy romprait la règle d'or |
...
| `ILiveTvPresenceProvider` (PR108) | `LiveTvPresenceProvider` (nouveau) | `IUserManager`, `ILocalizationManager`, `IUserViewFactory` | Non (chaîne transitive entièrement vérifiée §5) |
| `IUserViewCatalog` (PR109) | `UserViewCatalog` (nouveau) | `IUserRootFolderProvider`, `IUserViewFactory`, `IChannelCatalog`, `ILiveTvPresenceProvider`, `IItemSortService`, `IServerConfigurationManager`, `ILocalizationManager`, `IItemQueryService` (probe playlist/boxset, §3) | Non — tous les maillons ci-dessus sont individuellement prouvés cycle-free |

---

## 9. Design cible par PR (106–111)

### PR106 — Factory des vues, sans `ILibraryManager` ni `IUserViewManager`
- Introduire `IItemStore` (prérequis, §2/§8) : `Guid GetNewItemId(string, Type)`, `void CreateItem(BaseItem, BaseItem?)`, `void RegisterItem(BaseItem)`. Implémentation `ItemStore` composée de `IItemPersistenceService`/`IItemCacheStore`/`IServerConfigurationManager` (implémentations concrètes vérifiées §2 : `ItemPersistenceService`, `ItemLookupService`).
- Introduire `IUserViewFactory { GetNamedView(4 surcharges), GetShadowView }`, implémentation `UserViewFactory` — **`Lazy<IProviderManager>`, pas d'injection directe** (§2, `ProviderManager` référence `ILibraryManager`).
- Trancher le sort de l'événement `ItemAdded` (§2, §10 risque 6) avant de considérer PR106 clos : soit `IItemStore`/`UserViewFactory` expose un mécanisme équivalent, soit le changement de comportement (plus de notification `LibraryMonitor`/`LibraryChangedNotifier` à la création de vue) est documenté et couvert par un test de caractérisation explicite.
- `LibraryManager` délègue ses méthodes `GetNamedView`/`GetShadowView` (surface `ILibraryManager` inchangée) à `IUserViewFactory`.
- **Ne touche pas** `UserViewManager`/`LiveTvManager` (migration en PR106b implicite ou différée à PR109/PR110 selon séquencement — à trancher en implémentation, mais l'objectif de sortie de PR106 est que le leaf existe et soit testé en parité, pas que tous les appelants aient migré).
- **Critère de sortie mesurable** : `UserViewFactory` compile et passe des tests de parité contre `LibraryManager.GetNamedView`/`GetShadowView` historiques ; `UserViewFactory` n'a, dans son graphe de dépendances, aucune référence à `ILibraryManager`, `IUserViewManager`, `IChannelManager`, `ILiveTvManager` (vérifiable par `graphify path` depuis `UserViewFactory` vers ces 4 interfaces → aucun chemin direct par constructeur).
...
### PR111 — Clôture
- Mesurer : ≥1 `Lazy<T>` supprimé (`Lazy<IUserViewManager>`, confirmé §7.4/PR110) ; constructeur `LibraryManager` réduit d'au moins 3 paramètres — **voir réserve §7.5, ce critère n'est garanti que pour 1 paramètre par ce RFC seul, les 2 autres nécessitent un travail complémentaire non détaillé ici** ; une seule orchestration query (`IItemQueryService`, déjà vrai depuis PR86) ; une seule implémentation du catalogue de vues (`UserViewCatalog`, `UserViewManager.GetUserViews` délègue) ; zéro service locator (aucun `IServiceProvider` dans ces nœuds — déjà vrai aujourd'hui, à ne pas régresser) ; façades historiques (`ILibraryManager.GetNamedView`/`GetShadowView`, `IUserViewManager.GetUserViews`) délèguent sans dupliquer la logique.

---

## 10. Risques et rollback

1. **Le −3 paramètres du ctor `LibraryManager` (PR111) n'est prouvé que pour −1 par ce RFC.** Risque : le critère de sortie du plan v13 pour PR111 peut ne pas être atteint sans travail supplémentaire (ex. extraction d'un autre `Lazy<T>` ou d'un service annexe hors périmètre direct de ce cycle). Mitigation : traiter ce critère comme un objectif best-effort cumulé sur PR106-111, pas comme garanti par la seule suppression de `Lazy<IUserViewManager>` ; si non atteint, documenter explicitement l'écart à la clôture (PR111) plutôt que de forcer une extraction artificielle.
...
6. **L'événement `ItemAdded` ne serait plus déclenché** si la création de vues named/shadow bascule de `LibraryManager.CreateItems` vers un `IItemStore`/`UserViewFactory` autonome (§2). Deux abonnés réels existent aujourd'hui : `LibraryMonitor` (`Reefin.Server.Core/IO/LibraryMonitor.cs:118`) et `LibraryChangedNotifier` (`Reefin.Server.Core/EntryPoints/LibraryChangedNotifier.cs:80`), tous deux abonnés sur l'instance `ILibraryManager`. Une classe `IItemStore` distincte ne peut pas lever cet événement sur cette instance. Mitigation : PR106 doit soit répliquer la notification (ex. `IItemStore` expose son propre événement/callback, `LibraryManager` s'y abonne et relaie sur son propre `ItemAdded` pour préserver le comportement observable), soit documenter explicitement le changement de comportement avec un test de caractérisation qui le rend visible (pas une régression silencieuse). Ne pas trancher cela avant PR106 est le principal risque de parité comportementale de ce RFC.
...
| --- | --- | --- |
| PR106 | `IItemStore` + `IUserViewFactory` (leaves, tests de parité) | Aucune arête vers les 4 membres de la SCC |
...
| PR111 | Clôture : mesures, façades délèguent, doc plan v13 mise à jour | Tous les critères §9/PR111, écarts documentés le cas échéant (voir §10.1) |
