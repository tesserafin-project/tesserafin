...
| PR115c | Branche le vrai chemin de streaming : pour une session canary-autoritaire avec un plan exécutable, construit le `StreamInfo` via l'adaptateur + le contexte au lieu du `StreamInfo` legacy ; fallback legacy sinon. | **Fusionné** — voir `docs/major-rewrite-plan-v13.md` §journal (PR115c) : `MediaInfoHelper.ResolveServedStreamInfo`, taxonomie `PlaybackLiveFallbackReason`, kill switch (`PlaybackShadowOptions.Mode`), exclusion Dolby Vision obligatoire, diagnostics `IPlaybackLiveWiringDiagnosticsStore`. |
| PR115d | Gate opérationnel avant ouverture du cohort en production (`CanaryPercentage` > 0 en environnement réel). | **Implémenté** — métriques agrégées (`PlaybackOperationalMetrics`), garde-fou de seuils d'arrêt (`PlaybackStopThresholdGuard`, `PlaybackStopThresholdOptions`), endpoint `GET /System/PlaybackDiagnostics/Metrics`, runbook de rollout `docs/pr115d-rollout-runbook.md`. |
