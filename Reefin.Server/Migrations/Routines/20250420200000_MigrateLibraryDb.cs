...
using Chapter = Reefin.Database.Implementations.Entities.Chapter;
...
/// </summary>
[ReefinMigration("2025-04-20T20:00:00", nameof(MigrateLibraryDb))]
[ReefinMigrationBackup(ReefinDb = true, LegacyLibraryDb = true)]
internal class MigrateLibraryDb : IDatabaseMigrationRoutine
{
    ...
    public void Perform()
    {
        ...
        {
            operation.ReefinDbContext.AttachmentStreamInfos.ExecuteDelete();
            operation.ReefinDbContext.BaseItems.ExecuteDelete();
            operation.ReefinDbContext.ItemValues.ExecuteDelete();
            operation.ReefinDbContext.UserData.ExecuteDelete();
            operation.ReefinDbContext.MediaStreamInfos.ExecuteDelete();
            operation.ReefinDbContext.Peoples.ExecuteDelete();
            operation.ReefinDbContext.PeopleBaseItemMap.ExecuteDelete();
            operation.ReefinDbContext.Chapters.ExecuteDelete();
            operation.ReefinDbContext.AncestorIds.ExecuteDelete();
        }
        ...
        using (var operation = GetPreparedDbContext("Moving TypedBaseItem"))
        {
            ...
            using (new TrackedMigrationStep("Clean TypedBaseItems hierarchy", _logger))
            {
                ...
                foreach (var item in allItemsLookup)
                {
                    ...
                    if (DoesResolve(cachedItem.BaseItem.ParentId, checkStack))
                    {
                        ...
                        operation.ReefinDbContext.BaseItems.Add(cachedItem.BaseItem);
            ...
            using (new TrackedMigrationStep($"Saving {operation.ReefinDbContext.BaseItems.Local.Count} BaseItem entries", _logger))
            {
                operation.ReefinDbContext.SaveChanges();
            }
        ...
        using (var operation = GetPreparedDbContext("Moving ItemValues"))
        {
            ...
            using (new TrackedMigrationStep("Loading ItemValues", _logger))
            {
                ...
                {
                    operation.ReefinDbContext.ItemValues.Add(item.Value.ItemValue);
                    operation.ReefinDbContext.ItemValuesMap.AddRange(item.Value.ItemIds.Distinct().Select(f => new ItemValueMap()
                    {
                        Item = null!,
                        ItemValue = null!,
                        ItemId = f,
            ...
            using (new TrackedMigrationStep($"Saving {operation.ReefinDbContext.ItemValues.Local.Count} ItemValues entries", _logger))
            {
                operation.ReefinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving UserData"))
        {
            ...
            {
                var users = operation.ReefinDbContext.Users.AsNoTracking().ToArray();
                ...
                foreach (var entity in queryResult)
                {
                    ...
                    operation.ReefinDbContext.UserData.Add(userData);
                }
            }

            legacyBaseItemWithUserKeys.Clear();

            using (new TrackedMigrationStep($"Saving {operation.ReefinDbContext.UserData.Local.Count} UserData entries", _logger))
            {
                operation.ReefinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving MediaStreamInfos"))
        {
            ...
            using (new TrackedMigrationStep("Loading MediaStreamInfos", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(mediaStreamQuery))
                {
                    ...
                    operation.ReefinDbContext.MediaStreamInfos.Add(entity);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.ReefinDbContext.MediaStreamInfos.Local.Count} MediaStreamInfos entries", _logger))
            {
                operation.ReefinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving AttachmentStreamInfos"))
        {
            ...
            using (new TrackedMigrationStep("Loading AttachmentStreamInfos", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(mediaAttachmentQuery))
                {
                    ...
                    operation.ReefinDbContext.AttachmentStreamInfos.Add(entity);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.ReefinDbContext.AttachmentStreamInfos.Local.Count} AttachmentStreamInfos entries", _logger))
            {
                operation.ReefinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving People"))
        {
            ...
            using (new TrackedMigrationStep("Loading People", _logger))
            {
                ...
                {
                    operation.ReefinDbContext.Peoples.Add(item.Value.Person);
                    operation.ReefinDbContext.PeopleBaseItemMap.AddRange(item.Value.Items.DistinctBy(e => (e.ItemId, e.PeopleId)));
                }

                peopleCache.Clear();
            }

            using (new TrackedMigrationStep($"Saving {operation.ReefinDbContext.Peoples.Local.Count} People entries and {operation.ReefinDbContext.PeopleBaseItemMap.Local.Count} maps", _logger))
            {
                operation.ReefinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving Chapters"))
        {
            ...
            using (new TrackedMigrationStep("Loading Chapters", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(chapterQuery))
                {
                    ...
                    operation.ReefinDbContext.Chapters.Add(chapter);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.ReefinDbContext.Chapters.Local.Count} Chapters entries", _logger))
            {
                operation.ReefinDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving AncestorIds"))
        {
            ...
            using (new TrackedMigrationStep("Loading AncestorIds", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(ancestorIdsQuery))
                {
                    ...
                    operation.ReefinDbContext.AncestorIds.Add(ancestorId);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.ReefinDbContext.AncestorIds.Local.Count} AncestorId entries", _logger))
            {
                operation.ReefinDbContext.SaveChanges();
            }
        }
    ...
    {
        public DatabaseMigrationStep(ReefinDbContext reefinDbContext, string operationName, ILogger logger) : base(operationName, logger)
        {
            ReefinDbContext = reefinDbContext;
        }

        public ReefinDbContext ReefinDbContext { get; }
