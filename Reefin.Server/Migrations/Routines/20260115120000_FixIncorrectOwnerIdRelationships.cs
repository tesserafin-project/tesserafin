...
/// </summary>
[ReefinMigration("2026-01-15T12:00:00", nameof(FixIncorrectOwnerIdRelationships))]
...
public class FixIncorrectOwnerIdRelationships : IAsyncMigrationRoutine
{
    ...
    private async Task RemoveDuplicateItemsAsync(ReefinDbContext context, CancellationToken cancellationToken)
    {
    ...
    private async Task ClearIncorrectOwnerIdsAsync(ReefinDbContext context, CancellationToken cancellationToken)
    {
    ...
    private async Task ReassignOrphanedExtrasAsync(ReefinDbContext context, CancellationToken cancellationToken)
    {
    ...
    private async Task PopulatePrimaryVersionIdAsync(ReefinDbContext context, CancellationToken cancellationToken)
    {
        ...
        var alternateVersionLinks = await context.LinkedChildren
            .Where(lc => (lc.ChildType == Reefin.Database.Implementations.Entities.LinkedChildType.LocalAlternateVersion
                       || lc.ChildType == Reefin.Database.Implementations.Entities.LinkedChildType.LinkedAlternateVersion))
            .Join(
                context.BaseItems,
                lc => lc.ChildId,
                item => item.Id,
