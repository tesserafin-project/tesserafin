...
namespace Reefin.Server.Integration.Tests
{
    ...
    /// </para>
    /// </summary>
    public sealed class OpenApiContractTests : IClassFixture<ReefinApplicationFactory>
    {
        ...
        public OpenApiContractTests(ReefinApplicationFactory factory, ITestOutputHelper outputHelper)
        {
            _factory = factory;
        ...
        public async Task Contract_IsByteIdentical_AcrossColdGenerations()
        {
            ...
            await using (var factoryA = new ReefinApplicationFactory())
            {
                first = await GenerateCanonicalAsync(factoryA);
            }

            await using (var factoryB = new ReefinApplicationFactory())
            {
        ...
        private static async Task<byte[]> GenerateCanonicalAsync(ReefinApplicationFactory factory)
        {
