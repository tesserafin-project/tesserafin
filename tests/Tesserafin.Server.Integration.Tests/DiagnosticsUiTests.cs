...
namespace Tesserafin.Server.Integration.Tests
{
    ...
    /// <see cref="Tesserafin.Api.Controllers.PlaybackDiagnosticsSessionsController"/>, which remains
    /// <c>[Authorize(Policy = Policies.RequiresElevation)]</c> - see
    /// <c>Tesserafin.Api.Tests.Controllers.PlaybackDiagnosticsSessionsControllerTests.Controller_RequiresElevation</c>
    ...
    /// </summary>
    public sealed class DiagnosticsUiTests : IClassFixture<TesserafinApplicationFactory>
    {
        private readonly TesserafinApplicationFactory _factory;

        public DiagnosticsUiTests(TesserafinApplicationFactory factory)
        {
            _factory = factory;
        }
