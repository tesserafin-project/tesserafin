...
public sealed class AlternateVersionQueryTranslationTests : IDisposable
{
    ...
    private static (Guid UserId, Guid PrimaryId, Guid VersionId, Guid OtherId) Seed(TesserafinDbContext ctx)
    {
        var user = new User("test", "auth-provider", "reset-provider");
        ctx.Users.Add(user);

        var primary = new BaseItemEntity { Id = Guid.NewGuid(), Type = "Tesserafin.Controller.Entities.Movies.Movie" };
        var version = new BaseItemEntity { Id = Guid.NewGuid(), Type = "Tesserafin.Controller.Entities.Movies.Movie", PrimaryVersionId = primary.Id };
        var other = new BaseItemEntity { Id = Guid.NewGuid(), Type = "Tesserafin.Controller.Entities.Movies.Movie" };
    ...
    private static (Guid UserId, Guid PrimaryId, Guid VersionAId, Guid VersionBId) SeedTiedVersions(TesserafinDbContext ctx)
    {
        var user = new User("test", "auth-provider", "reset-provider");
        ctx.Users.Add(user);

        var primary = new BaseItemEntity { Id = Guid.NewGuid(), Type = "Tesserafin.Controller.Entities.Movies.Movie" };
        var versionA = new BaseItemEntity { Id = Guid.NewGuid(), Type = "Tesserafin.Controller.Entities.Movies.Movie", PrimaryVersionId = primary.Id };
        var versionB = new BaseItemEntity { Id = Guid.NewGuid(), Type = "Tesserafin.Controller.Entities.Movies.Movie", PrimaryVersionId = primary.Id };
