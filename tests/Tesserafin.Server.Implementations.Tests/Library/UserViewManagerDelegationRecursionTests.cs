...
/// around the *same* real <see cref="Tesserafin.Server.Core.Library.UserViewManager"/> instance under
...
public sealed class UserViewManagerDelegationRecursionTests
{
    ...
    public void GetUserViews_PlaylistsLibrary_FullDelegationChain_DoesNotRecurse()
    {
        ...
        configMock.Setup(c => c.Configuration).Returns(new Tesserafin.Model.Configuration.ServerConfiguration());
        BaseItem.ConfigurationManager = configMock.Object;
        BaseItem.LibraryManager = Mock.Of<ILibraryManager>();

        var localizationMock = new Mock<Tesserafin.Model.Globalization.ILocalizationManager>();

        var itemSortService = new Tesserafin.Server.Core.Sorting.ItemSortService(Mock.Of<IUserManager>(), Mock.Of<IUserDataManager>());
        ...
        var userViewFactory = new Tesserafin.Server.Core.Library.UserViewFactory(
            itemLookupService,
            itemStore,
            configMock.Object,
        ...
        var itemQueryServiceFactory = new Lazy<IItemQueryService>(() => new Tesserafin.Server.Core.Library.ItemQueryService(
        ...
        var realManager = new Tesserafin.Server.Core.Library.UserViewManager(
