...
namespace Tesserafin.Server.Implementations.Fuzz
{
    public static class Program
    {
        ...
        private static SqliteItemRepository MockSqliteItemRepository()
        {
            ...
            configSection.SetupGet(x => x[It.Is<string>(s => s == Tesserafin.Controller.Extensions.ConfigurationExtensions.SqliteCacheSizeKey)])
                .Returns("0");
            var config = new Mock<IConfiguration>();
            config.Setup(x => x.GetSection(It.Is<string>(s => s == Tesserafin.Controller.Extensions.ConfigurationExtensions.SqliteCacheSizeKey)))
