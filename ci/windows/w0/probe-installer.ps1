...
function New-DeterminismMsi {
    ...
    # ProductCode is pinned rather than left to WiX's default autogeneration --
    # an autogenerated ProductCode is exactly the kind of per-build difference
    # this experiment exists to isolate away from. Safe here ONLY because every
    # build in this experiment is the same version: the lifecycle WXS above
    # deliberately does NOT do this, because a real major upgrade needs a fresh
    # ProductCode per version.
    #
    # It is `ProductCode=`, not `Id=`. In WiX v4 and later `Package/@Id` is an
    # IDENTIFIER (letters, digits, underscores, periods) and rejects a GUID with
    # WIX0014; the ProductCode GUID has its own attribute. Measured, not assumed
    # -- the first run of this experiment failed exactly that way.
    $wxs = @"
<Wix xmlns="http://wixtoolset.org/schemas/v4/wxs">
  <Package ProductCode="6f1e9d2b-8a4c-4e77-9b1a-3c5d7e9f2a01"
...
if ($determinismFacts.buildA.exitCode -ne 0 -or $determinismFacts.buildB.exitCode -ne 0) {
    Add-W0Fact -Evidence $evidence -Id 'installer.msi.determinism' -Bucket 'blocked' `
        -Detail ("WiX $WixVersion could not build the determinism probe MSI twice; no comparison is " +
                 "possible. buildA exit=$($determinismFacts.buildA.exitCode) " +
                 "buildB exit=$($determinismFacts.buildB.exitCode).") `
        -Data $determinismFacts
    Save-W0Evidence -Evidence $evidence -Path (Join-Path $EvidenceDir 'installer.json') | Out-Null
    # Recording the gap and carrying on would leave a green job whose central
    # question was never asked -- the exact failure mode this harness is being
    # repaired to remove.
    throw ("W0 HARD STOP: the MSI determinism experiment did not run. " +
           "buildA=$($determinismFacts.buildA.exitCode) buildB=$($determinismFacts.buildB.exitCode); " +
           "see installer.json for the compiler output.")
} else {
    ...
    # actual toolset rather than asserted from MSI folklore. WiX v4+ exposes no
    # Package/@PackageCode attribute; compiling one is the proof, and WIX0004
    # ("unexpected attribute") is the answer.
    $packageCodeProbe = Join-Path $WorkRoot 'packagecode-probe'
    New-Item -ItemType Directory -Force -Path $packageCodeProbe | Out-Null
    $packageCodeWxs = Join-Path $packageCodeProbe 'packagecode.wxs'
    Set-Content -LiteralPath $packageCodeWxs -Encoding utf8NoBOM -Value @"
<Wix xmlns="http://wixtoolset.org/schemas/v4/wxs">
  <Package PackageCode="6f1e9d2b-8a4c-4e77-9b1a-3c5d7e9f2a02"
           ProductCode="6f1e9d2b-8a4c-4e77-9b1a-3c5d7e9f2a01"
           Name="Tesserafin W0 PackageCode probe"
           Manufacturer="Tesserafin W0 (disposable probe, not a product)"
           Version="1.0.0"
           UpgradeCode="9a2c4e6f-1b3d-4a5c-8e7f-2d4b6a8c0e21"
...
"@
    $packageCodeLog = & wix build -arch x64 -intermediatefolder $packageCodeProbe `
        -o (Join-Path $packageCodeProbe 'packagecode.msi') $packageCodeWxs 2>&1 | Out-String
    $determinismFacts.packageCodeSettable = ($LASTEXITCODE -eq 0)
    $determinismFacts.packageCodeProbeLog = $packageCodeLog.Trim()
