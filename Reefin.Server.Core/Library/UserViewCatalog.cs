...
namespace Reefin.Server.Core.Library
{
    ...
    /// (<c>Reefin.Server.Core/Library/ItemQueryService.cs:38</c>) - it needs them for its own
    ...
    /// <see cref="IItemQueryService.GetItemList(Folder, Reefin.Controller.Entities.InternalItemsQuery)"/> - if not); not
