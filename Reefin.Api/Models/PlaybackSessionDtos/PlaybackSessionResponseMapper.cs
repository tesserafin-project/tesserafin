...
/// <c>Reefin.Playback.Shadow.LegacyDecisionProjector</c> (PR98/PR111c) — same method mapping, same
...
/// <see cref="Reefin.Playback.Decision"/> vocabulary this response exposes, and never onto that
...
/// <c>LegacyDecisionProjector</c> documents. Likewise, <see cref="Reefin.Model.Dlna.StreamInfo"/>
...
public static class PlaybackSessionResponseMapper
{
    ...
    private static SelectedSubtitle? MapSelectedSubtitle(StreamInfo streamInfo)
    {
        ...
        // Reefin.Model.Dlna.SubtitleDeliveryMethod and Reefin.Playback.Decision.SubtitleDeliveryMethod
        ...
        {
            LegacySubtitleDeliveryMethod.Encode => Reefin.Playback.Decision.SubtitleDeliveryMethod.Burn,
            LegacySubtitleDeliveryMethod.Embed => Reefin.Playback.Decision.SubtitleDeliveryMethod.Embed,
            LegacySubtitleDeliveryMethod.External => Reefin.Playback.Decision.SubtitleDeliveryMethod.External,
            LegacySubtitleDeliveryMethod.Hls => Reefin.Playback.Decision.SubtitleDeliveryMethod.Hls,
            // Drop means the subtitle is not actually delivered to the client - equivalent to no
            // selection for this response's purposes.
            _ => (Reefin.Playback.Decision.SubtitleDeliveryMethod?)null,
        };

        return delivery is Reefin.Playback.Decision.SubtitleDeliveryMethod method ? new SelectedSubtitle(index, method) : null;
    }
