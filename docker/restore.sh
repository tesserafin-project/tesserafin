...
# Verifies the SHA-256 sidecar (if present), refuses to clobber a non-empty target
# unless --force, extracts through a helper container, and re-asserts the runtime
# ownership (uid:gid 10000:10000) so the non-root server can read/write its state.
#
...
CONTAINER="" ; FORCE=0 ; HELPER="busybox:stable"
...
docker run --rm \
  ...
  -v "${ARCHIVE_DIR}:/in:ro" \
  "${HELPER}" sh -eu -c '
    if [ "'"${FORCE}"'" = "1" ]; then rm -rf /v/config/* /v/data/* 2>/dev/null || true; fi
