...
using Tesserafin.Api.Models.LibraryDtos;
...
/// </summary>
[Route("")]
public class LibraryController : BaseTesserafinApiController
{
        ...
        [FromQuery, ModelBinder(typeof(CommaDelimitedCollectionModelBinder))] SortOrder[]? sortOrder = null)
    {
        ...
        while (true)
        {
            themeItems = _itemSortService.Sort(
                item.GetExtras(user).Where(e => e.ExtraType == Tesserafin.Model.Entities.ExtraType.ThemeSong),
                user,
                orderBy).ToArray();
        ...
        [FromQuery, ModelBinder(typeof(CommaDelimitedCollectionModelBinder))] SortOrder[]? sortOrder = null)
    {
        ...
        {
            themeItems = _itemSortService.Sort(
                item.GetExtras(user).Where(e => e.ExtraType == Tesserafin.Model.Entities.ExtraType.ThemeVideo),
                user,
                orderBy).ToArray();
    ...
    public ActionResult PostUpdatedSeries([FromQuery] string? tvdbId)
    {
        ...
        }).Where(i => string.Equals(tvdbId, i.GetProviderId(Tesserafin.Model.Entities.MetadataProvider.Tvdb), StringComparison.OrdinalIgnoreCase)).ToArray();
    ...
    public ActionResult PostUpdatedMovies([FromQuery] string? tmdbId, [FromQuery] string? imdbId)
    {
        ...
        {
            movies = movies.Where(i => string.Equals(imdbId, i.GetProviderId(Tesserafin.Model.Entities.MetadataProvider.Imdb), StringComparison.OrdinalIgnoreCase)).ToList();
        }
        else if (!string.IsNullOrWhiteSpace(tmdbId))
        {
            movies = movies.Where(i => string.Equals(tmdbId, i.GetProviderId(Tesserafin.Model.Entities.MetadataProvider.Tmdb), StringComparison.OrdinalIgnoreCase)).ToList();
        }
        else
        {
