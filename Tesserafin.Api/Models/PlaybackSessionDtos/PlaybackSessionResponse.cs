...
/// <c>Tesserafin.Controller.MediaEncoding.PlaybackSession</c> record. Every property is either a
/// primitive or drawn from the <see cref="Tesserafin.Playback.Decision"/> vocabulary (PR91) — no
/// <c>Tesserafin.Model.Dlna</c> type (<c>DeviceProfile</c>, <c>MediaOptions</c>, <c>StreamInfo</c>)
...
/// <c>Map(PlaybackSession, Tesserafin.MediaEncoding.Playback.V2PlanRecord?)</c> overload) — otherwise
...
/// the constraint codes that mirror <c>Tesserafin.Model.Session.TranscodeReason</c> one-to-one can be
