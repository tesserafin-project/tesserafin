...
using DlnaSubtitleDeliveryMethod = Tesserafin.Model.Dlna.SubtitleDeliveryMethod;
...
public static class ClientCapabilitiesMapper
{
    ...
    private static DecodeCapabilities BuildDecodeCapabilities(DeviceProfile profile)
    {
        ...
        var supportsHls = profile.TranscodingProfiles.Any(static t => t.Protocol == Tesserafin.Data.Enums.MediaStreamProtocol.hls);

        // Tesserafin.Data.Enums.MediaStreamProtocol has no "dash" member (only http/hls), so a
    ...
    private static IReadOnlyList<PlaybackOutputProfile> BuildOutputProfiles(DeviceProfile profile)
    {
        ...
        foreach (var transcodingProfile in profile.TranscodingProfiles)
        {
            ...
            var protocol = transcodingProfile.Protocol == Tesserafin.Data.Enums.MediaStreamProtocol.hls
