...
```cmd
dotnet ef migrations add {MIGRATION_NAME} --project "src/Reefin.Database/Reefin.Database.Providers.Sqlite" --output-dir Migrations -- --migration-provider Reefin-SQLite
```

If you get the error: `Run "dotnet tool restore" to make the "dotnet-ef" command available.` Run `dotnet restore`.

in the event that you get the error: `System.UnauthorizedAccessException: Access to the path '/src/Reefin.Database' is denied.` you have to restore as sudo and then run `ef migrations` as sudo too.
